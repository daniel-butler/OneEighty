...
//  OneEightyTests
//
//  Tests for OneEightyEngine state management, BPM persistence, and playback.
//
...
final class OneEightyEngineTests: XCTestCase {
    ...
    func testSetupRestoresBPMFromSharedState() {
        ...
        let engine1 = OneEightyEngine(store: store)
        engine1.setup()
        engine1.setBPM(210)
        engine1.teardown()
    ...
    func testBPMRange() {
        engine.setup()
        XCTAssertEqual(OneEightyEngine.bpmRange, 150...230)
    }
    ...
    func testEngineUsesInjectedStore() {
        let store = InMemoryStateStore()
        store.bpm = 200
        let injectedEngine = OneEightyEngine(store: store)
